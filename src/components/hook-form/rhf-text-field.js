...
export default function RHFTextField({ name, helperText, type, inputSteps, inputType, ...other }) {
            ...
            endAdornment: inputType === 'password' && (
              <InputAdornment position="end">
                <IconButton onClick={isSecure.onToggle}>
                  <Iconify icon={isSecure.value ? 'eva:eye-off-outline' : 'eva:eye-outline'} />
                </IconButton>
              </InputAdornment>
            ),
          }}
                ...
                <TextField
                  ...
                  onChange={(event) => {
                    ...
                    if (value.length === 3 && index < Number(inputSteps) - 1) {
                      const nextInput = event.target.parentNode.parentNode.nextSibling?.nextSibling;
