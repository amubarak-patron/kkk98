        ...
        if (["select", "radio-group", "upload"].includes(field?.type)) {
          return {
            ...
            renderRow: (row) => {
              ...
              return <Typography variant="body2" noWrap>{row[labelId + '_label'] || "---"}</Typography>;
            },
          };
          ...
          return {
            ...
            renderRow: (row) => {
              return <Typography variant="body2" noWrap style={{
                direction: "ltr"
              }}>{row[field?.id || field?.fieldVariable]}</Typography>;
            },
          };
        }
        ...
        .map((field) => {
          if (["select", "radio-group", "upload"].includes(field?.type)) {
            return {
              ...
              renderRow: (row) => {
                return <Typography variant="body2" noWrap>{row[field?.fieldVariable + '_label'] || "---"}</Typography>;
              },
            };
          } else if (["phonefield"].includes(field?.type)) {
            return {
